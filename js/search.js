...
const DELAY = 500; // 0.5 sec debounce
...
function isMatch(item, text) {
  var result = false;
  for(var key in item) {
    if(typeof item[key] === 'object') result = result || isMatch(item[key], text);
    else result = result || item[key].toLowerCase().indexOf(text.toLowerCase()) > -1;
  }
  return result;
}
...
function typeAhead(inputElement, allAvailableOptions) {
  var currentFocus;
  inputElement.addEventListener("input", debounce(inputChangeHandler, DELAY));
